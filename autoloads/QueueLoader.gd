...
"""
Based on https://github.com/replicantstudio/godot_background_loader
"""
...
func _process(delta):
	...
	if err == ERR_FILE_EOF:
		...
		_log("Resource loaded: '%s' at %s" % [_curr_id, _curr_path])
		_clear_curr_loader()

	# If error has occurred
	elif err != OK:
		_log("Error: Couldn't load '%s' at %s" % [_curr_id, _curr_path])
		_clear_curr_loader()
